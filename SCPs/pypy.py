...
def process_treatment_summary(treatment_ws, json_path, start_row=1):
    ...
    # Helper function to write subsection headers
    def write_subsection(title, row):
        treatment_ws.merge_cells(start_row=row, start_column=1, end_row=row, end_column=7)
        treatment_ws.cell(row=row, column=1, value=title)
        treatment_ws.cell(row=row, column=1).font = Font(bold=True, size=12)
        treatment_ws.cell(row=row, column=1).fill = subsection_fill
        treatment_ws.cell(row=row, column=1).alignment = Alignment(horizontal='left', vertical='center', wrap_text=True)
        return row + 1
    ...
    # 1. Diagnosis
    current_row = write_subsection("Diagnosis", current_row)
    ...
    if diagnosis_list and isinstance(diagnosis_list, list):
        ...
        current_row += 2  # Add a blank row after subsection
    else:
        print(f"No diagnosis data found in '{json_path}'.")
        current_row += 2  # Add spacing even if data is missing
    ...
    current_row = write_subsection("Surgery Conducted", current_row)
    treatment_ws.cell(row=current_row, column=1, value=surgery_conducted)
    current_row += 2
    ...
    if surgery_info_list and isinstance(surgery_info_list, list):
        ...
        surgery_info = {}
    current_row = write_subsection("Surgery Information", current_row)
    ...
    current_row = write_subsection("Radiation Treatment Conducted", current_row)
    treatment_ws.cell(row=current_row, column=1, value=radiation_conducted)
    current_row += 2
    ...
    if radiation_info_list and isinstance(radiation_info_list, list):
        ...
        radiation_info = {}
    current_row = write_subsection("Radiation Treatment Information", current_row)
    treatment_ws.cell(row=current_row, column=1, value="Procedure")
    ...
    current_row = write_subsection("Systemic Therapy Conducted (Chemotherapy, hormonal therapy, other)", current_row)
    treatment_ws.cell(row=current_row, column=1, value=systemic_conducted)
    current_row += 2

    # 7. Agents Used in Completed Treatments
    current_row = write_subsection("Agents Used in Completed Treatments", current_row)
    ...
    current_row = write_subsection("Persistent Symptoms or Side Effects at Completion of Treatment", current_row)
    treatment_ws.cell(row=current_row, column=1, value=persistent_symptoms)
    current_row += 2

    # 9. Symptoms or Side Effects
    current_row = write_subsection("Symptoms or Side Effects", current_row)
    ...
    if need_ongoing_list and isinstance(need_ongoing_list, list):
        ...
        need_ongoing = "N/A"
    current_row = write_subsection("Need for Ongoing (Adjuvant) Treatment for Cancer", current_row)
    treatment_ws.cell(row=current_row, column=1, value=need_ongoing)
    current_row += 2
    ...
    if ongoing_info_list and isinstance(ongoing_info_list, list):
        ...
        ongoing_info = {}
    current_row = write_subsection("Ongoing Treatment Information", current_row)
    ...
    current_row = write_subsection("Additional Comments", current_row)
    additional_comments = data.get("Additional Comments", "")
    treatment_ws.cell(row=current_row, column=1, value=sanitize_text(additional_comments))
    treatment_ws.cell(row=current_row, column=1).alignment = Alignment(wrap_text=True)
    current_row +=1
...
for i in PATIENT_INDICES:
    ...
    for main_file, context_file in main_to_context_files.items():
        ...
        patient_current_row = write_section(patient_ws, information_category, main_color, patient_current_row)

        # Depending on the main_file, process accordingly
        if main_file == f'{i}_Already experienced symptoms or side effects.json':
            # Processing '0_Already experienced symptoms or side effects.json'
            # Adjust the keys based on your actual JSON structure
            # Example structure assumed here
            ...
            patient_current_row = write_subsection(patient_ws, current_section, patient_current_row)
            ...
            patient_ws.cell(row=patient_current_row, column=1).font = Font(bold=True)
            patient_ws.cell(row=patient_current_row, column=1).alignment = Alignment(wrap_text=True)
            patient_current_row +=2  # Blank row after section

        elif main_file == f'{i}_Cancer surveillance and other recommended tests for cancer monitoring.json':
            # Processing '0_Cancer surveillance and other recommended tests for cancer monitoring.json'
            ...
            if tests and isinstance(tests, list):
                ...
                for test in tests:
                    ...
                    if patient_current_row % 2 == 0:
                        ...
                        fill_color = "FFFFFFFF"  # White for odd rows
                    patient_ws.cell(row=patient_current_row, column=1).fill = PatternFill(start_color=fill_color, end_color=fill_color, fill_type="solid")
                    patient_ws.cell(row=patient_current_row, column=2).fill = PatternFill(start_color=fill_color, end_color=fill_color, fill_type="solid")

                    patient_current_row +=2  # Blank row after each test
            else:
            ...
            # Processing '0_Lifestyle and behavior recommendations for cancer survivors.json'
            ...
            if recommendations and isinstance(recommendations, list):
                ...
                for rec in recommendations:
                    ...
                    patient_current_row +=2  # Blank row after each recommendation
            else:
            ...
            # Processing '0_Possible late and long-term effects of cancer treatment.json'
            ...
            if effects and isinstance(effects, list):
                ...
                for effect in effects:
                    ...
                    patient_current_row +=2  # Blank row after each effect
            else:
            ...
            # Processing '0_Possible other issues that cancer survivors may experience.json'
            ...
            if issues and isinstance(issues, list):
                ...
                for issue in issues:
                    ...
                    patient_current_row +=2  # Blank row after each issue
            else:
            ...
            # Processing '0_References to helpful resources for cancer survivors.json'
            ...
            if references and isinstance(references, list):
                ...
                for ref in references:
                    ...
                    patient_current_row +=2  # Blank row after each reference
            else:
